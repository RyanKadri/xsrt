...
export class DomManager {
  ...
  createInitialDocument(data: SnapshotChunk) {
    ...
    if (head) {
      (head as OptimizedHtmlElementInfo).children.unshift({
        id: -1, type: "element", tag: "style", children: [{
          type: "text",
  ...
  private replaceReferences(valWithRefs: string, references: number[]) {
    references.forEach(ref => {
      ...
      valWithRefs = valWithRefs.replace(formatAssetRef(ref), proxyAsset.hostedPath ?? proxyUrl ?? proxyAsset.origUrl);
    });
    return valWithRefs;
  }
