export const LocationSymbol = Symbol("Location");
export const DocumentSymbol = Symbol("Document");
export const WindowSymbol = Symbol("Window");
export const LocalStorageSymbol = Symbol("LocalStorage");
export const GotSymbol = Symbol("Got");
export const FetchSymbol = Symbol("Fetch")
export const ContainerSymbol = Symbol("Container");
export const DBConnectionSymbol = Symbol("Connection");
export const RabbitChannelSymbol = Symbol("RabbitChannel");
export const SQSConnectionSymbol = Symbol("SqsSymbol");
export const RabbitChunkQueueSymbol = Symbol("RabbitChunkQueue");
